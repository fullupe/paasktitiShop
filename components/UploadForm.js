...
function UploadForm() {
    ...
    return (
        <div>
                ...
                <button onClick={(ent)=>router.push('/customerOrders')} className="bg-cyan-500 p-2 rounded-md focus:outline-none focus:ring-2 hover:bg-cyan-300">Manage Order's </button>
                </div>
        ...
        {/* <div className="p-4 py-4 bg-cyan-500"><button onClick={(ent)=>router.push('/customerOrders')} className="">Manage </button></div> */}
            <input className="font-bold text-gray-800  rounded-full bg-white outline-none shadow-lg " type="file"  onChange={changeHandler}/>
            <div>
                { error && <div>{ error }</div> }
                { file && <div>{ file.name }</div> }
                {file && <ProgressBar imageUrl={imageUrl} setFile={setFile} progress={progress}/>}
            ...
            {/*  */}
              ...
              <div>
                <label for="" className="text-sm ">Product Description</label>
                  <input required ref={descriptionInputRef } type="text" placeholder="Description"
             ...
             <div>
                <label for="" className="text-sm">Product Price</label>
                <input required ref={priceInputRef} type="number" placeholder="Price" 
            ...
            <button disabled={!file || !priceInputRef || !descriptionInputRef}  className="inline-block  rounded-lg bg-cyan-800 text-white font-bold px-6 py-2 uppercase text-sm" onClick={handlerSumit}>send</button>
